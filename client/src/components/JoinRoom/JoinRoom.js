...
const JoinRoom = (props) => {
  ...
  const searchRooms = async (query) => {
    if (query.trim().length < 1) {
      ...
      if (roomSearchList.status === 200) {
        dispatch({
          type: "handleRoomSearch",
          roomSearchList: roomSearchList.data,
        });
      }
    }
  };
  ...
  const requestJoin = async (name, owner) => {
    ...
    let response = await roomService.sendJoinRequest(
      name,
      owner,
      loggedUsername
    );
    if (response.status === 204) {
      dispatch({
        type: "handleRoomRequestSuccess",
        requestRoom: { name: name, owner: owner },
      });
      ...
      <Card>
        <Card.Body>
          {!props.hasRooms ? (
            <>
              <Card.Title>
                Looks like you're not a part of any rooms yet.
              </Card.Title>
            ...
            <>
              ...
              <Card.Text>Or make a request to join another room!</Card.Text>
            </>
          )}
          <Form.Group>
                  ...
                  return (
                    <>
                      <ListGroup.Item
                        key={room.name}
                        onClick={() => {
                          requestJoin(room.name, room.owner);
                        }}
                        className="search-res-wrapper"
                      >
                        <span className="search-res name">{room.name}</span>
                        <span className="search-res owner text-muted">
                          {room.owner}
                        </span>
                      </ListGroup.Item>
                    </>
